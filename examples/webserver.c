							...
							else
							{
								...
								if(reqh->range.start)
								{
									if(reqh->range.start>jbl_string_get_length(res)||reqh->range.end>jbl_string_get_length(res))
										resh=jwl_http_head_set_status		(resh,416),jbl_log(UC "[%d,%d) out of [%d,%d)",reqh->range.start,reqh->range.end,0LL,jbl_string_get_length(res)),res=jbl_string_free(res);
									else
									{
